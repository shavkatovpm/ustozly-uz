...
export function VacancyFilters({ total }: { total: number }) {
  ...
  const activeCount = [subject, region, format, employment].filter(Boolean).length;
      ...
      >
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Lavozim, fan yoki markaz nomi"
            className="pl-10"
            aria-label="Qidirish"
          />
        </div>
        ...
        <div className="min-h-0">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 p-3 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)]">
            ...
            <FilterSelect
              ...
              options={[
                { value: "all", label: "Barchasi" },
                ...employmentTypes.map((e) => ({ value: e.value, label: e.label })),
              ]}
            />
          </div>
        </div>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="inline-flex items-center gap-2 text-sm">
          ...
          <Select value={sort} onValueChange={(v) => setParam("sort", v)}>
            ...
            <SelectContent>
              <SelectItem value="top">Eng munosib</SelectItem>
              <SelectItem value="recent">Eng yangi</SelectItem>
              <SelectItem value="salary-desc">Maosh kamayish</SelectItem>
              <SelectItem value="salary-asc">Maosh oshish</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
